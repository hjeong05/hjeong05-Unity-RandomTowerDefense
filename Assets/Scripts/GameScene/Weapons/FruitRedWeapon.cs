...
public class FruitRedWeapon : MonoBehaviour {
    ...
    {
        bulletPrefab.GetComponent<FruitRedBullets>().damage = bulletPrefab.GetComponent<FruitRedBullets>().StartDamage;

    }
    void Start()
    {
        float up = bulletPrefab.GetComponent<FruitRedBullets>().GetDamage();      // 강화시 데미지 업 
        bulletPrefab.GetComponent<FruitRedBullets>().SetDamage(up);
    ...
    {
        float up = bulletPrefab.GetComponent<FruitRedBullets>().GetDamage() + 2f;
        bulletPrefab.GetComponent<FruitRedBullets>().SetDamage(up);
        Debug.Log(this.gameObject.name + "빨강 무기 중간에 강화 데미지: " + up);
    }
    void Update()
    {
        if (IsDestroy == true)
    ...
    void Shoot()
    {
        ...
        FruitRedBullets bullet = bulletSpawn1.GetComponent<FruitRedBullets>();
