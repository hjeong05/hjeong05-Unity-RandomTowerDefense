...
public class FruitOrangeWeapon : MonoBehaviour {
    ...
    {
        bulletPrefab.GetComponent<FruitOrangeBullets>().damage = bulletPrefab.GetComponent<FruitOrangeBullets>().StartDamage;
    }
    void Start()
    {

        float up = bulletPrefab.GetComponent<FruitOrangeBullets>().GetDamage();      // 강화시 데미지 업 
        bulletPrefab.GetComponent<FruitOrangeBullets>().SetDamage(up);
        IsDestroy = false;
        firstTerm = fireTerm;
    ...
    {
        float up = bulletPrefab.GetComponent<FruitOrangeBullets>().GetDamage() + 2f;
        bulletPrefab.GetComponent<FruitOrangeBullets>().SetDamage(up);
        Debug.Log(this.gameObject.name + "열매 오렌지 무기 중간에 강화 데미지: " + up);
    }
    void Update()
    {            
        if (IsDestroy == true)
    ...
    //    Debug.Log(this.gameObject.tag);
        ...
        FruitOrangeBullets bullet = bulletSpawn1.GetComponent<FruitOrangeBullets>();
