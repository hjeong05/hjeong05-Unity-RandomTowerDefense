...
public class YellowWeapon : MonoBehaviour {
    ...
    void Start()
    {
        ...
        float up = bulletPrefab.GetComponent<YellowBullets>().GetDamage();      // 강화시 데미지 업 
        bulletPrefab.GetComponent<YellowBullets>().SetDamage(up);
        Debug.Log("노란색 무기 시작 데미지: " + up);
        IsDestroy = false;
        firstTerm = fireTerm;
    ...
    {
        bulletPrefab.GetComponent<YellowBullets>().damage = bulletPrefab.GetComponent<YellowBullets>().StartDamage;
    }
    ...
    {
        float up = bulletPrefab.GetComponent<YellowBullets>().GetDamage() + damage;
        bulletPrefab.GetComponent<YellowBullets>().SetDamage(up);
        Debug.Log(this.gameObject.name + "노랑 무기 중간에 강화 데미지: " + up);
    }
    void Update()
    {
        if (IsDestroy == true)
  ...
  //      Debug.Log(this.gameObject.tag);
        ...
        if (firePoint2 != null)
        {
            ...
            YellowBullets bullet2 = bulletSpawn2.GetComponent<YellowBullets>();

            if (bullet2 != null)
                bullet2.Find(target);
        }

        YellowBullets bullet = bulletSpawn1.GetComponent<YellowBullets>();
